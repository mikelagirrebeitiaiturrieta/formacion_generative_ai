...
def demo_transform():
    st.header = "Tranasform"
    # Add a select box
    options = [TASK_BULLET_POINTS,TASK_HTML_FORMAT]
    selected_option = st.selectbox("Select an option:", options, index=0)


    sample_text1 = use_case_transform.get_sample_text(selected_option)
    complete_prompt1 = use_case_transform.get_prompt(sample_text1, selected_option)
    st.write(complete_prompt1)

    transform_button = st.button("Transform")

    # Notify the user that we ran the update:
    if transform_button:
        response = use_case_transform.transform(api_key,watsonx_project_id,complete_prompt1,selected_option,selected_use_case_model)
...
def main():
    ...
    selected_option = st.sidebar.selectbox("Select Option", [OPTION_SUMMARY,OPTION_EXTRACT,OPTION_ANALYZE,OPTION_TRANSFORMATION])
    ...
    if selected_option == OPTION_SUMMARY:
        ...
        demo_analyze()
    elif selected_option == OPTION_TRANSFORMATION:
        demo_transform()
